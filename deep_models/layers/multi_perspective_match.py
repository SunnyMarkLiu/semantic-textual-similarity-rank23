...
class MultiPerspectiveMatch(Layer):
    ...
    This code is on https://github.com/faneshion/MatchZoo/blob/master/matchzoo/layers/MultiPerspectiveMatch.py
    """
    ...
    def call(self, inputs, **kwargs):
        ...
        q_rep, q_last, d_rep, d_last = inputs
        outputs = []
...
def cal_cosine_weighted_q_rep(q_rep, cosine_matrix, normalize=False, eps=1e-6):
    ...
    if not normalize:
        weighted_question_words = K.tf.div(weighted_question_words,
                                           K.tf.expand_dims(K.tf.add(K.tf.reduce_sum(cosine_matrix, axis=-1), eps),
                                                            axis=-1))
