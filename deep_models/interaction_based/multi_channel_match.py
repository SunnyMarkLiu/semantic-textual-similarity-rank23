...
from keras.layers import Dropout, BatchNormalization, Reshape, Flatten, concatenate, Bidirectional, LSTM, Dot, GRU, CuDNNGRU
...
class MultiChannelMatch(BaseModel):
    ...
    def build_model(self, data):
        ...
        """
        """Input layers"""
        m1_q1_input = Input(shape=(self.cfg.max_sequence_length,), dtype='int32', name='m1_q1_input')
        m1_q2_input = Input(shape=(self.cfg.max_sequence_length,), dtype='int32', name='m1_q2_input')
        m2_q1_input = Input(shape=(self.cfg.max_sequence_length,), dtype='int32', name='m2_q1_input')
        m2_q2_input = Input(shape=(self.cfg.max_sequence_length,), dtype='int32', name='m2_q2_input')
        m3_q1_input = Input(shape=(self.cfg.max_sequence_length,), dtype='int32', name='m3_q1_input')
        m3_q2_input = Input(shape=(self.cfg.max_sequence_length,), dtype='int32', name='m3_q2_input')
        ...
        if self.cfg.multi_channel_match_cfg['simple_architecture']:
            ...
            represent_dim = self.cfg.multi_channel_match_cfg['mlp_dense_units'] + merge_cnn_out_shape + self.cfg.multi_channel_match_cfg['rnn_units']

            mse_diff = Reshape((represent_dim, 1))(mse_diff)
            mul_diff = Reshape((represent_dim, 1))(mul_diff)
